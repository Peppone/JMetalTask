...
public class TaskProblem extends Problem {
	 ...
	 * @param t
	 */
	public TaskProblem(int task, int server, int serverPerRack, int rackPerPod,double pServerPeak,
			double pTorPeak, double pAggPeak,
			double/*[]*/ serverLinkMaximumBandwidth,
			double/*[]*/ rackLinkMaximumBandwidth,
			double cpuInstructionPerSecond/*[]*/, ArrayList<Task> t) {
		...
		this.rackNum = (serverNum / serverPerRack)
				+ (serverNum % serverPerRack == 0 ? 0 : 1);
		this.podNum = (rackNum / rackPerPod)
				+ (rackNum % rackPerPod == 0 ? 0 : 1);
	...
	public void evaluate(Solution solution) throws JMException {
		...
		double powerConsumption = serverNum * pServerIdle;
		...
		for (int i = 0; i < variable.length; ++i) {
			...
			if (bandwidthPerServer[server] > serverLinkMaximumBandwidth[server]) {
				bw = bandwidthPerServer[server]
						- serverLinkMaximumBandwidth[server];
				/*
		...
		double makespan = computeMakespan(instructionPerServer);
		powerConsumption += computeSwitchPowerConsumption(bandwidthPerRack,
				bandwidthPerPod);
	...
	private double computeSwitchPowerConsumption(double bandwidthPerRack[],
			double bandwidthPerPod[]) {
		...
		for (int i = 0; i < rack; ++i) {
			if (bandwidthPerRack[i] != 0) {
				tor_switch_power_consumption += pTorSwitchIdle
						+ (pTorSwitchPeak - pTorSwitchIdle)
	...
	private double computeMakespan(int[] instructionPerServer) {
		double makespan = -1;
		for (int i = 0; i < serverNum; ++i) {
			double serverTime = (1.0 * instructionPerServer[i])
					/ cpuInstructionPerSecond[i];
